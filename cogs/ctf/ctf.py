...
import discord
from lib.config import AUTOGENERATED_CHANNELS, ARCHIVE_CATEGORY, INTERNAL_CTFD_URL, CTF_ROLE_ID, UIUC_ROLE_ID, CTF_PREFIX
import requests
...
class CTF(commands.Cog):
    ...
    async def addrole(self, ctx: SlashContext, who: OptionType.MENTIONABLE) -> None:
        ...
        await logs_channel.set_permissions(to_add, send_messages=False, view_channel=True)
    ...
    async def removerole(self, ctx: SlashContext, who: OptionType.MENTIONABLE) -> None:
        ...
        if to_remove is None:
            ...
            who = f'&{who}'
        await category_folder.set_permissions(
            to_remove, view_channel=False, connect=False)
        logs_channel = discord.utils.get(
            category_folder.text_channels, name=AUTOGENERATED_CHANNELS['logs'])
        access_channel = discord.utils.get(
            category_folder.text_channels, name=AUTOGENERATED_CHANNELS['access'])
        await logs_channel.set_permissions(to_remove, send_messages=False, view_channel=False)
        await access_channel.set_permissions(to_remove, send_messages=False, view_channel=False)
